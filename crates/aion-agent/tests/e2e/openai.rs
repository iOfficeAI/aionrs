...
async fn test_openai_single_turn_completion() {
    ...
    eprintln!("[e2e] openai single-turn: {} tokens in / {} out",
...
async fn test_openai_tool_use() {
    ...
    assert!(
        ...
        "model should echo the content or use multiple turns: {}", result.text
    );

    eprintln!("[e2e] openai tool-use: {} turns, {} tokens out",
